...
class SephiaBot implements IRCListener {
  ...
  private long nextWho = 0;
  private long nextHi = 0;

  public static void main(String args[]) {

    SephiaBot sephiaBot = new SephiaBot("sephiabot.cfg");
    sephiaBot.connect();
  ...
  public SephiaBot(String config) {
    ...
    try {
      syslog = new BufferedWriter(new FileWriter("/home/vino/sephiabot/syslog.txt", false));
    } catch (IOException ioe) {
      logerror("Couldn't open syslog file.");
    }

    log("SephiaBot created.");

    parseConfig("sephiabot.cfg");
  ...
  void parseConfig(String name) {
    ...
    try {
      configIn = new BufferedReader(new FileReader("/home/vino/sephiabot/" + name));
    } catch (IOException ioe) {
      logerror("Couldn't open cfg file " + name + ".");
      return;
    }
...
/*        } else if (command.equals("spell")) {
      ...
      logerror("Couldn't read cfg file " + name + ".");
    }
  ...
  public void messagePrivMsg(String nick, String host, String recipient, String msg) {
    ...
    if (msg.toLowerCase().indexOf(name.toLowerCase()) != -1) {
      if (msg.toLowerCase().indexOf("fuck you") != -1) {
        if (System.currentTimeMillis() > nextWho) {	//Keep SephiaBot from being spammed.
    ...
    if (msg.toLowerCase().indexOf(name.substring(0, nameEnd).toLowerCase()) != -1) {
      for (int i = 0; i < hellos.length; i++) {
        if (msg.toLowerCase().startsWith(hellos[i])) {
          if (System.currentTimeMillis() > nextHi) {	//Keep SephiaBot from being spammed.
...
//      ircio.privmsg(recipient, nick + ", " + spell);
  ...
  public void log(String log) {
    try {
      ...
      System.out.println("Couldn't write to syslog!");
    }
...
class IRCIO {
  ...
  public IRCIO(IRCListener listener, String network, int port) {
    ...
    try {
      ...
      listener.logerror("IO Exception.");
      System.exit(0);
    }
